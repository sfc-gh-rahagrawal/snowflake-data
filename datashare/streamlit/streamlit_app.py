...
if not st.session_state.database_selected:
    st.markdown("---")
    st.markdown("### Step 1: Select Source Database")
    db_list = get_db_list()
    if not db_list:
    ...
    d_name_selected = st.selectbox(
        ...
        format_func=lambda x: "Select a database..." if x == "" else x
    )
    ...
    st.markdown(f"### Step 1: Selected Database: `{st.session_state.source_db_name}`")
...
if st.session_state.database_selected:
    st.markdown("### Step 2: Select Tables")
...
if st.session_state.selected_tables:
    st.markdown("---")
    st.markdown("### Step 3: Configure Table Transformations")
...
if st.session_state.database_selected:
    st.markdown("---")
    st.markdown("### Step 4: Review and Download Final Configuration")

    final_json_output = {
        "database_name": st.session_state.source_db_name,
        "share_created_by": current_user,
...
if st.session_state.source_db_name and st.session_state.selected_tables:
    st.markdown("---")
    st.markdown("### Step 5: Create Data Share")
